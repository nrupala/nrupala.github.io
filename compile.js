...
async function main() {
  ...
  const allRepos = response.data.filter(repo => 
  ...
  allRepos.forEach(repo => {
    ...
    for(let i = 0; i <= 6; i++) {
      const x = (i / 6) * 400;
      const y = Math.max(5, Math.min(40, 42 - (base * 0.7) - (Math.sin(i * 1.5) * (base * 0.2))));
      points.push(`${x},${y}`);
    }

    cardsHtml += `
      <div class="repo-card" data-name="${repo.name.toLowerCase()}" data-desc="${(repo.description || '').toLowerCase()}" data-lang="${(repo.language || 'config').toLowerCase()}">
          ...
          <div class="repo-desc">${repo.description || 'Production build deployment active.'}</div>
          <div class="graph-container">
              <svg viewBox="0 0 400 45" style="width:100%; height:100%; overflow:visible;">
                  <polyline fill="none" stroke="#7ee787" stroke-width="2" points="${points.join(' ')}" />
              </svg>
          </div>
          <div class="stats-panel">
              ...
              <div><div class="stat-val">${activityIdx}</div><div class="stat-lbl">Activity Index</div></div>
          </div>
          <div class="meta-footer">
              <span class="lang-badge">${repo.language || 'Config'}</span>
  ...
  <head>
      ...
      <style>
          ...
          .search-input { flex: 1; min-width: 200px; background: var(--bg-color); border: 1px solid var(--border-color); color: #fff; padding: 10px 15px; border-radius: 6px; }
  ...
  <body>
      ...
      <div class="controls-panel">
          ...
          <select id="langFilter" class="filter-select">${options}</select>
      </div>
      <div id="repo-grid">${cardsHtml}</div>
      <script>
